...
// ==========================================
// 3. INTERFACE PRINCIPAL G2 CRM PRO
// ==========================================
export default function App() {
  ...
  // ==========================================
  // 4. INTEGRAÇÃO COM O BANCO DE DADOS (SUPABASE REST)
  // ==========================================
  const fetchCustomers = async () => {
    ...
    } catch (err) { console.error("Erro ao ler contatos do Postgres:", err); }
  ...
  const fetchProducts = async () => {
    ...
    } catch (err) { console.error("Erro ao ler catálogo de produtos:", err); }
  };
  ...
  const handleDeleteProductFromSupabase = async (id: string) => {
    if (!confirm("Deseja realmente remover este infoproduto permanentemente do banco relacional?")) return;
    try {
  ...
  const handleUpdateCSMetrics = async (id: string, extraProgress: number, changeTickets: number) => {
    ...
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/contatos?id=eq.${id}`, {
        method: 'PATCH',
        headers: { "apikey": SUPABASE_ANON_KEY, "Authorization": `Bearer ${SUPABASE_ANON_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({ lessons_watched_percentage: newPct, open_tickets_count: newTickets, health_score: finalHealth })
      });
      fetchCustomers();
      if (selectedCustomer && selectedCustomer.id === id) {
        setSelectedCustomer(prev => prev ? { ...prev, lessons_watched_percentage: newPct, open_tickets_count: newTickets, health_score: finalHealth } : null);
      }
    } catch (err) { console.error(err); }
  };
  ...
  const handleSimulateWebhook = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        alert(`⚡ Gateway Hotmart Sincronizado!\n\nCompra mapeada no valor de R$ ${parsed.data.purchase.price.value}. Mariana Oliveira foi matriculada no Postgres.`);
        fetchCustomers();
      }
    } catch (err) { alert("JSON Payload inválido."); }
  };
    ...
    <div style={{ display: 'flex', backgroundColor: '#f8fafc', color: '#1e293b', minHeight: '100vh', userSelect: 'none', fontFamily: '"Inter", sans-serif' }}>
      ...
      <main style={{ flex: 1, padding: '40px', overflowX: 'auto', backgroundColor: '#f8fafc' }}>
        ...
        {currentTab === 'dashboard' && (
          <div>
            ...
            <p style={{ color: '#64748b', fontSize: '14px', margin: '4px 0 25px 0' }}>Métricas financeiras reais corporativas e controle de retenção da base G2</p>
            ...
            <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
              <h4 style={{ margin: '0 0 15px 0', fontSize: '14px', fontWeight: '700', color: '#0f172a' }}>Distribuição de Saúde (Fase Pós-Venda)</h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ fontSize: '13px', color: '#475569', fontWeight: '600' }}>Excelente Saúde ({'>'}=80): {clientesExcelente} | Zona de Alerta (50-79): {customers.length - clientesExcelente - clientesChurnRisco} | Churn Crítico: {clientesChurnRisco}</div>
                <div style={{ width: '100%', backgroundColor: '#f1f5f9', height: '14px', borderRadius: '10px', display: 'flex', overflow: 'hidden' }}>
                  <div style={{ width: `${(clientesExcelente/Math.max(1, customers.length))*100}%`, backgroundColor: '#10b981', transition: '0.3s' }}></div>
                  <div style={{ width: `${((customers.length - clientesExcelente - clientesChurnRisco)/Math.max(1, customers.length))*100}%`, backgroundColor: '#f59e0b', transition: '0.3s' }}></div>
                  <div style={{ width: `${(clientesChurnRisco/Math.max(1, customers.length))*100}%`, backgroundColor: '#ef4444', transition: '0.3s' }}></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <header style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '20px', marginBottom: '25px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px', backgroundColor: '#e2e8f0', padding: '4px', borderRadius: '8px', width: 'fit-content' }}>
                {funnels.map(funnel => (
                  <button key={funnel} onClick={() => setCurrentFunnel(funnel)} style={{ padding: '8px 16px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: '700', backgroundColor: currentFunnel === funnel ? '#0f172a' : 'transparent', color: currentFunnel === funnel ? '#ffffff' : '#475569', transition: '0.15s' }}>
                    {funnel}
                  </button>
                ))}
              </div>
            ...
            {isLoading ? (
              <div style={{ padding: '50px', textAlign: 'center', color: '#64748b', fontWeight: '600' }}>Conectando e lendo Tabelas PostgreSQL...</div>
            ) : (
                ...
                {activeStages.map(stage => {
                  ...
                  return (
                    <div key={stage} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, stage)} style={{ backgroundColor: '#f1f5f9', borderRadius: '12px', padding: '16px', minHeight: '520px', display: 'flex', flexDirection: 'column', border: '1px solid #e2e8f0' }}>
                      <h3 style={{ fontSize: '11px', fontWeight: '700', color: '#1e293b', display: 'flex', justifyContent: 'space-between', margin: '0 0 16px 0', letterSpacing: '0.5px' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', flex: 1 }}>
                        {stageCustomers.map(customer => (
                          <div key={customer.id} draggable onDragStart={(e) => handleDragStart(e, customer.id)} onClick={() => setSelectedCustomer(customer)} style={{ backgroundColor: '#ffffff', padding: '14px', borderRadius: '8px', border: '1px solid #e2e8f0', cursor: 'grab', position: 'relative', boxShadow: '0 1px 3px rgba(0,0,0,0.01)' }}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e2e8f0', paddingBottom: '20px', marginBottom: '25px' }}>
              <div>
                ...
                <p style={{ color: '#64748b', fontSize: '13px', margin: '4px 0 0 0' }}>Base relacional de alunos armazenada na nuvem da infraestrutura G2</p>
              </div>
              <button onClick={() => setShowAddModal(true)} style={{ backgroundColor: '#c5a880', color: '#0f172a', border: 'none', padding: '10px 20px', borderRadius: '6px', fontWeight: '700', cursor: 'pointer', fontSize: '12px' }}>
                ➕ Novo Contato Real
              </button>
        ...
        {currentTab === 'produtos' && (
          <div>
            ...
            <div style={{ backgroundColor: '#fdf2e9', border: '1px solid #fed7d7', padding: '16px', borderRadius: '8px', marginBottom: '25px', fontSize: '13px', color: '#c2410c', lineHeight: '1.5' }}>
              <strong>Mapeamento Automático:</strong> os webhooks externos vindos da Hotmart dão gatilho e cadastram os infoprodutos aqui de forma relacional automaticamente.
            </div>

            <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                ...
                <tbody>
                  {productsList.map(p => (
                    <tr key={p.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                      <td style={{ padding: '14px', fontWeight: '700', color: '#0f172a' }}>{p.crmName}</td>
                      <td style={{ padding: '14px', fontFamily: 'monospace', color: '#475569' }}>{p.hotmartId}</td>
                      <td style={{ padding: '14px' }}><span style={{ fontSize: '11px', backgroundColor: '#fbf7f0', color: '#c5a880', padding: '2px 8px', borderRadius: '4px', fontWeight: 'bold', border: '1px solid #e2e8f0' }}>{p.status}</span></td>
                      <td style={{ padding: '14px' }}><button onClick={() => handleDeleteProductFromSupabase(p.id)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontWeight: 'bold' }}>Remover do Banco</button></td>
                    </tr>
                  ))}
                  {productsList.length === 0 && <tr><td colSpan={4} style={{ padding: '24px', textAlign: 'center', color: '#94a3b8' }}>Nenhum produto cadastrado no banco relacional.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        {currentTab === 'integracoes' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
            <div>
              ...
              <p style={{ color: '#64748b', fontSize: '13px', margin: '4px 0 20px 0' }}>Simule a entrada de webhooks de plataformas de infoprodutos</p>
              
              <form onSubmit={handleSimulateWebhook} style={{ backgroundColor: '#ffffff', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                <span style={{ fontSize: '12px', fontWeight: '700', display: 'block', marginBottom: '6px' }}>Payload JSON de Entrada (Hotmart Mock API)</span>
                ...
                <button type="submit" style={{ width: '100%', marginTop: '12px', backgroundColor: '#0f172a', color: '#c5a880', fontWeight: '700', padding: '12px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '13px' }}>Simular Post Real no Postgres</button>
              </form>
            </div>
            <div>
              <h2 style={{ fontSize: '24px', fontWeight: '700', color: '#0f172a', margin: '0 0 20px 0' }}>Chaves Privadas</h2>
              <div style={{ backgroundColor: '#ffffff', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0', marginBottom: '15px' }}><span style={{ fontSize: '11px', color: '#64748b', fontWeight: 'bold' }}>HOTMART WEBHOOK SECRET</span><input type="password" value={hotmartToken} onChange={(e) => setHotmartToken(e.target.value)} style={{ width: '95%', padding: '8px', borderRadius: '6px', marginTop: '6px', border: '1px solid #cbd5e1' }} /></div>
              <div style={{ backgroundColor: '#ffffff', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0' }}><span style={{ fontSize: '11px', color: '#64748b', fontWeight: 'bold' }}>BEE MESSAGE GATEWAY TOKEN</span><input type="password" value={beeMessageKey} onChange={(e) => setBeeMessageKey(e.target.value)} style={{ width: '95%', padding: '8px', borderRadius: '6px', marginTop: '6px', border: '1px solid #cbd5e1' }} /></div>
            </div>
          </div>
        )}
        ...
        {currentTab === 'configuracoes' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '30px', maxWidth: '700px' }}>
            ...
            <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              ...
              <h4 style={{ fontSize: '12px', fontWeight: '700', color: '#64748b', marginBottom: '8px' }}>Membros com Permissão Ativa ({usersList.length}):</h4>
              {usersList.map(user => <div key={user} style={{ padding: '10px 0', borderBottom: '1px solid #f1f5f9', fontSize: '13px', fontWeight: '500' }}>• {user}</div>)}
            </div>
          </div>
        )}
      ...
      {showProductModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(15, 23, 42, 0.4)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 200, backdropFilter: 'blur(3px)' }}>
          <form onSubmit={handleSaveProduct} style={{ backgroundColor: '#ffffff', padding: '30px', borderRadius: '16px', width: '380px', border: '1px solid #e2e8f0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}><h2 style={{ fontSize: '16px', fontWeight: '700', color: '#0f172a', margin: 0 }}>Mapear Novo Infoproduto</h2><button type="button" onClick={() => setShowProductModal(false)} style={{ background: 'none', border: 'none', color: '#ef4444', fontSize: '18px', cursor: 'pointer' }}>✕</button></div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', fontSize: '12px' }}>
              <div><label style={{ display: 'block', marginBottom: '6px', fontWeight: '600' }}>Nome de Exibição CRM *</label><input type="text" value={prodCrmName} onChange={(e) => setProdCrmName(e.target.value)} style={{ width: '94%', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1' }} required placeholder="Ex: Mentoria Pro" /></div>
              <div><label style={{ display: 'block', marginBottom: '6px', fontWeight: '600' }}>ID do Gatilho Hotmart *</label><input type="text" value={prodHotmartId} onChange={(e) => setProdHotmartId(e.target.value)} style={{ width: '94%', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1' }} required placeholder="Ex: HOT-MENTORIA" /></div>
            </div>
            <button type="submit" style={{ width: '100%', marginTop: '25px', padding: '12px', borderRadius: '6px', backgroundColor: '#0f172a', color: '#c5a880', fontWeight: '700', border: 'none', cursor: 'pointer' }}>Vincular Produto G2</button>
          </form>
        </div>
      )}
